...
const buildGraphData = (relations) => {
    ...
    relations.forEach((rel) => {
        if (rel.source_id && !seenNodes.has(rel.source_id)) {
            ...
            nodes.push({ id: rel.source_id, label: rel.source_label || rel.source_id, title: rel.source_label || rel.source_id, color: "#1d4ed8" });
        }
        if (rel.target_id && !seenNodes.has(rel.target_id)) {
            seenNodes.set(rel.target_id, true);
            nodes.push({ id: rel.target_id, label: rel.target_label || rel.target_id, title: rel.target_label || rel.target_id, color: "#9333ea" });
        }
        edges.push({
            id: `edge-${rel.id}`, from: rel.source_id, to: rel.target_id,
...
const applyCircularLayout = (nodes) => {
    ...
    return nodes.map((node, idx) => {
        ...
        return { ...node, x: radius * Math.cos(angle), y: radius * Math.sin(angle), fixed: true };
    });
};

const presetOptions = (layout) => {
    if (layout === "circular") return { layout: { improvedLayout: false }, physics: { enabled: false } };
    if (layout === "kamada_kawai") return { layout: { improvedLayout: true }, physics: { enabled: true, solver: "forceAtlas2Based", stabilization: { fit: true, iterations: 200 } } };
    return { layout: { improvedLayout: true }, physics: { enabled: true, solver: "barnesHut", stabilization: { fit: true, iterations: 200 } } };
};
...
const renderNetwork = (data) => {
    ...
    const visData = { nodes: new vis.DataSet(nodes), edges: new vis.DataSet(data.edges) };
    if (!network) {
...
const loadGraph = async () => {
    ...
    try {
        const limitVal = parseInt(limitInput.value, 10) || 500;
        const relationType = relationTypeInput.value.trim() || null;
        const data = await api.fetchAllRelations(limitVal, relationType);
        if (!Array.isArray(data) || data.length === 0) {
            setStatus("No relations found for this filter.");
...
(async () => {
    try {
        await api.initDB((msg) => { if (loadingEl) loadingEl.textContent = msg; });
        if (loadingEl) loadingEl.style.display = 'none';
        await loadGraph();
    } catch (err) {
        if (loadingEl) loadingEl.textContent = 'DB init failed: ' + String(err);
    }
})();
