...
<head>
    ...
    <meta property="og:type"
        content="website">
    <meta property="og:url" content="/assets/background-generator/node_modules/ts-node/dist-raw/">
    ...
    <script type="application/ld+json">
    {
      ...
      "mainEntityOfPage": {
        "@type": "WebPage",
        "@id": "/assets/background-generator/node_modules/ts-node/dist-raw/"
      },
      "headline": "Index",
...
<body>
    ...
    <header class="site-header">
        ...
        <nav class="language-switcher" aria-label="Language selector">
            ...
            <a href="../../../../../de/assets/background-generator/node_modules/ts-node/dist-raw/" class="">Deutsch</a>
    ...
    <div id="main-container">
        <div class="toc-container">
            <ul class="toc-list ">
                ...
                <li>
                    <a href="#raw-directory">raw directory</a>
        ...
        <div class="content">
            ...
            <p>The <code>dist-raw</code> directory contains JS sources that are distributed verbatim, not compiled nor typechecked via TS.</p>
<p>To implement ESM support, we unfortunately must duplicate some of node's built-in functionality that is not
exposed via an API.  We have copy-pasted the necessary code from https://github.com/nodejs/node/tree/master/lib
then modified it to suite our needs.</p>
<p>Formatting may be intentionally bad to keep the diff as small as possible, to make it easier to merge
upstream changes and understand our modifications.  For example, when we need to wrap node's source code
in a factory function, we will not indent the function body, to avoid whitespace changes in the diff.</p>
<p>One obvious problem with this approach: the code has been pulled from one version of node, whereas users of ts-node
run multiple versions of node.
Users running node 12 may see that ts-node behaves like node 14, for example.</p>
<h2 id="raw-directory"><code>raw</code> directory</h2>
<p>Within the <code>raw</code> directory, we keep unmodified copies of the node source files.  This allows us to use diffing tools to
compare files in <code>raw</code> to those in <code>dist-raw</code>, which will highlight all of the changes we have made.  Hopefully, these
changes are as minimal as possible.</p>
...
<p>In the <code>raw</code> directory, files are suffixed with the version number or revision from which
they were downloaded.</p>
<p>If they have a <code>stripped</code> suffix, this means they have large chunks of code deleted, but no other modifications.
This is useful when diffing.  Sometimes our <code>dist-raw</code> files only have a small part of a much larger node source file.
It is easier to diff <code>raw/*-stripped.js</code> against <code>dist-raw/*.js</code>.</p>
        </div>
    </div>
